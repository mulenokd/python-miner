...
def clearance(ids):
    ...
    c.itemconfig(ids, fill="#bfbfbf") # окрашиваем клетку в зеленый
        ...
        while len(neigh_list) > 0:
            ...
            if item_around > 0:
                ...
                if item not in clicked:
                    ...
                    c.create_text(x1 + SQUARE_SIZE / 1.5,
                                  y1 + SQUARE_SIZE / 1.5,
                                  text=str(item_around),
                                  font="Arial {}".format(int(SQUARE_SIZE / 1.5)),
                                  fill='black')
...
def rec_clearance(ids):
    # Рекурсивная функция очистки поля
    clicked.add(ids)
    neighbors = generate_neighbors(ids)
    around = check_mines(neighbors)
    if around:
        x1, y1, x2, y2 = c.coords(ids)
        c.itemconfig(ids, fill="#bfbfbf")
        c.create_text(x1 + SQUARE_SIZE / 1.5,
                      y1 + SQUARE_SIZE / 1.5,
                      text=str(around),
                      font="Arial {}".format(int(SQUARE_SIZE / 1.5)),
...
def click(event):
    ...
    if ids in mines:
        ...
        clearance(ids)
        c.itemconfig(CURRENT, fill="#bfbfbf")
    c.update()
...
for i in range(GRID_SIZE):
    for j in range(GRID_SIZE):
      c.create_rectangle(i * SQUARE_SIZE, j * SQUARE_SIZE,
                         ...
                         j * SQUARE_SIZE + SQUARE_SIZE, fill='#e6e6e6')
root.mainloop()